...
class Imenu extends Model
{
    ...
    protected $rule = [
        'imenu_name|菜单名称'     => 'require|unique:menu',
    ];
     ...
     **/
    public function GetListByPage($where=array(), $page=1, $limit=100, $order='imenu_rank desc')
    {   
        return $this->where($where)->page($page,$limit)->order($order)->select();
    }

    //获取数据列表，不分页
    public function GetDataList($where=array(), $order='imenu_rank desc')
    {
    ...
    {
        $res = $this->allowField(true)->save($param, ['imenu_id' => $param['imenu_id']]);
