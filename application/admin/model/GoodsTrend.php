 ...
 * 商品趋势 表数据模型
 **/
class GoodsTrend extends Model
{
    //声明主键
    protected $pk = 'gt_id';
    //自动写入时间戳
     ...
     **/
    public function GetListByPage($where=array(), $page=1, $limit=10, $order="gt_id desc")
    {   
        return $this->where($where)->page($page,$limit)->order($order)->select();
    }

    //获取数据列表，不分页
    public function GetDataList($where=array(), $order="gt_id desc")
    {
