...
{
    //声明主键
    protected $pk = 'curation_id';
    //自动写入时间戳
    protected $autoWriteTimestamp = true;
    //声明添加时间字段
    protected $createTime = 'curation_time';
    //声明修改时间字段
    //protected $updateTime = 'rotate_time';
    //关闭自动写入
    protected $updateTime = false;
    //声明表名
    protected $table = 'm15_vip_duration';
    //声明只读字段 该字段写入后不可被修改
    //protected $readonly = ['name','email'];
    protected $rule = [
        'curation_level|特权信息'       => 'require',
    ];
     ...
     **/
    public function GetListByPage($where=array(), $page=1, $limit=10, $order="curation_id desc")
    {   
        return $this->where($where)->page($page,$limit)->order($order)->select();
    }

    //获取数据列表，不分页
    public function GetDataList($where=array(), $order="curation_id desc")
    {
