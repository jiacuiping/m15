 ...
 * 模版 表数据模型
 **/
class VideoTrend extends Model
{
    //声明主键
    protected $pk = 'vt_id';
    //自动写入时间戳
    ...
    //声明添加时间字段
    protected $createTime = 'vt_time';
    //声明修改时间字段
    //protected $updateTime = 'frame_time';
    //关闭自动写入
    protected $updateTime = false;
    //声明表名
    protected $table = 'm15_video_trend';
    protected $rule = [
        'vt_video_id|关联视频'       => 'require',
        'vt_video_number|视频ID'    => 'require',
    ];
     ...
     **/
    public function GetListByPage($where=array(), $page=1, $limit=10, $order="vt_id desc")
    {   
        return $this->where($where)->page($page,$limit)->order($order)->select();
    }

    //获取数据列表，不分页
    public function GetDataList($where=array(), $order="vt_id desc")
    {
