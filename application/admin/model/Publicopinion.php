...
class Publicopinion extends Model
{
    ...
    //声明表名
    protected $table = 'm15_publicopinion';
    protected $rule = [
        'public_type|舆情类型'   => 'require',
        'public_key|关联主键'    => 'require',
    ];
     ...
     **/
    public function GetListByPage($where=array(), $page=1, $limit=10, $order="public_id desc")
    {   
        return $this->where($where)->page($page,$limit)->order($order)->select();
    }

    //获取数据列表，不分页
    public function GetDataList($where=array(), $order="public_id desc")
    {
