...
class Video extends Model
{
    ...
    //声明添加时间字段
    protected $createTime = 'video_time';
    //声明修改时间字段
    ...
    protected $rule = [
        'video_platform|所属平台'     => 'require|number',
        'video_number|视频ID'       => 'require|unique:video'
    ];
     ...
     **/
    public function GetListByPage($where=array(), $page=1, $limit=10, $order="video_id desc")
    {   
        return $this->where($where)->page($page,$limit)->order($order)->select();
    }

    //获取数据列表，不分页
    public function GetDataList($where=array(), $order="video_id desc")
    {
