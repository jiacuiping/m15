...
class UserAccount extends Model
{
    ...
    //声明添加时间字段
    protected $createTime = 'account_time';
    //声明修改时间字段
    ...
    protected $rule = [
        'account_user|关联用户'     => 'require|number',
        'account_kol|关联KOL'      => 'require|number',
    ];
     ...
     **/
    public function GetListByPage($where=array(), $page=1, $limit=10, $order="account_id desc")
    {   
        return $this->where($where)->page($page,$limit)->order($order)->select();
    }

    //获取数据列表，不分页
    public function GetDataList($where=array(), $order="account_id desc")
    {
