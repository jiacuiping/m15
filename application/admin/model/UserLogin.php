...
class UserLogin extends Model
{
    ...
    //声明表名
    protected $table = 'm15_user_login';
    protected $rule = [
        'login_name|关联用户'        => 'require|unique:user_login|max:10',
        'login_mobile|登陆手机号'    => 'require|number|unique:user_login|length:11',
    ];
     ...
     **/
    public function GetListByPage($where=array(), $page=1, $limit=10, $order="login_id desc")
    {   
        return $this->where($where)->page($page,$limit)->order($order)->select();
    }

    //获取数据列表，不分页
    public function GetDataList($where=array(), $order="login_id desc")
    {
