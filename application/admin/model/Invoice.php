...
class Invoice extends Model
{
    ...
    //声明添加时间字段
    protected $createTime = 'invoice_time';

    protected $rule = [
        'invoice_user'       => 'require',
    ];
     ...
     **/
    public function GetListByPage($where=array(), $page=1, $limit=10, $order="invoice_id desc")
    {   
        return $this->where($where)->page($page,$limit)->order($order)->select();
    }

    //获取数据列表，不分页
    public function GetDataList($where=array(), $order="invoice_id desc", $field = "*")
    {
