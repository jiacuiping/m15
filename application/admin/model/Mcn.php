...
class Mcn extends Model
{
    ...
    protected $rule = [
        'mcn_name|MCN名称'        => 'require',
        'mcn_authname|MCN认证名称' => 'require',
    ];
     ...
     **/
    public function GetListByPage($where=array(), $page=1, $limit=10, $order="mcn_id desc")
    {   
        return $this->where($where)->page($page,$limit)->order($order)->select();
    }

    //获取数据列表，不分页
    public function GetDataList($where=array(), $order="mcn_id desc")
    {
