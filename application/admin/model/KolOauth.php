...
class KolOauth extends Model
{
    ...
    //声明添加时间字段
    protected $createTime = 'oauth_time';
    //声明修改时间字段
    //protected $updateTime = 'frame_time';
    //关闭自动写入
    protected $updateTime = false;
    //声明表名
    protected $table = 'm15_kol_oauth';
    protected $rule = [
        'oauth_kol|关联kolid'       => 'require',
    ];
     ...
     **/
    public function GetListByPage($where=array(), $page=1, $limit=10, $order="oauth_id desc")
    {   
        return $this->where($where)->page($page,$limit)->order($order)->select();
    }

    //获取数据列表，不分页
    public function GetDataList($where=array(), $order="oauth_id desc")
    {
    ...
    public function UpdateData($param)
    {
        ...
        $data = $this->GetOneData([$this->pk => $param[$this->pk]]);
