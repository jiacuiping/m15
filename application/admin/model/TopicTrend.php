 ...
 * 模版 表数据模型
 **/
class TopicTrend extends Model
{
    //声明主键
    protected $pk = 'tt_id';
    //自动写入时间戳
    ...
    //声明添加时间字段
    protected $createTime = 'tt_time';
    //声明修改时间字段
    //protected $updateTime = 'frame_time';
    //关闭自动写入
    protected $updateTime = false;
    //声明表名
    protected $table = 'm15_topic_trend';
    protected $rule = [
        'tt_topic_id|关联话题'       => 'require',
        'tt_topic_number|话题ID'    => 'require',
    ];
     ...
     **/
    public function GetListByPage($where=array(), $page=1, $limit=10, $order="tt_id desc")
    {   
        return $this->where($where)->page($page,$limit)->order($order)->select();
    }

    //获取数据列表，不分页
    public function GetDataList($where=array(), $order="tt_id desc")
    {
