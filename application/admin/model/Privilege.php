...
class Privilege extends Model
{
    ...
    //声明添加时间字段
    protected $createTime = 'level_time';
    //声明修改时间字段
    //protected $updateTime = 'rotate_time';
    //关闭自动写入
    protected $updateTime = false;
    //声明表名
    protected $table = 'm15_vip_level';
    //声明只读字段 该字段写入后不可被修改
    //protected $readonly = ['name','email'];
    protected $rule = [
        'level_name|特权名称'       => 'require',
    ];
     ...
     **/
    public function GetListByPage($where=array(), $page=1, $limit=10, $order="level_id desc")
    {   
        return $this->where($where)->page($page,$limit)->order($order)->select();
    }

    //获取数据列表，不分页
    public function GetDataList($where=array(), $order="level_id desc")
    {
