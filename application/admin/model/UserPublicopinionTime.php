 ...
 * 模版 表数据模型
 **/
class UserPublicopinionTime extends Model
{
    //声明主键
    protected $pk = 'time_id';
    //自动写入时间戳
    ...
    //声明表名
    protected $table = 'm15_user_publicopinion_time';
    protected $rule = [
        'time_kol|关联kol'   => 'require',
    ];
     ...
     **/
    public function GetListByPage($where=array(), $page=1, $limit=10, $order="time_id desc")
    {   
        return $this->where($where)->page($page,$limit)->order($order)->select();
    }

    //获取数据列表，不分页
    public function GetDataList($where=array(), $order="time_id desc")
    {
     ...
     **/
    public function GetOneData($where=array(), $order="create_time desc")
    {
