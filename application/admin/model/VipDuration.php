 ...
 * 特权等级时间 表数据模型
 **/
class VipDuration extends Model
{
    //声明主键
    protected $pk = 'duration_id';
    //自动写入时间戳
    protected $autoWriteTimestamp = true;
    //声明添加时间字段
    protected $createTime = 'duration_time';
    //声明修改时间字段
    ...
    protected $rule = [
        'duration_level|关联vip_level表'     => 'require',
    ];
     ...
     **/
    public function GetListByPage($where=array(), $page=1, $limit=10, $order="duration_id desc")
    {   
        return $this->where($where)->page($page,$limit)->order($order)->select();
    }

    //获取数据列表，不分页
    public function GetDataList($where=array(), $order="duration_id desc")
    {
