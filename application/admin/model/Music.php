...
class Music extends Model
{
    ...
    //声明添加时间字段
    protected $createTime = 'music_time';
    //声明修改时间字段
    ...
    protected $rule = [
        'music_platform|所属平台'     => 'require|number',
        'music_number|音乐ID'       => 'require|unique:music'
    ];
     ...
     **/
    public function GetListByPage($where=array(), $page=1, $limit=10, $order="music_id desc")
    {   
        return $this->where($where)->page($page,$limit)->order($order)->select();
    }

    //获取数据列表，不分页
    public function GetDataList($where=array(), $order="music_id desc")
    {
