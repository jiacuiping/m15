...
class VideoComment extends Model
{
    ...
    //声明添加时间字段
    protected $createTime = 'comm_time';
    //声明修改时间字段
    //protected $updateTime = 'frame_time';
    //关闭自动写入
    protected $updateTime = false;
    //声明表名
    protected $table = 'm15_video_comment';
    protected $rule = [
        'comm_video|关联视频'       => 'require',
    ];
     ...
     **/
    public function GetListByPage($where=array(), $page=1, $limit=10, $order="comm_like desc")
    {   
        return $this->where($where)->page($page,$limit)->order($order)->select();
    }

    //获取数据列表，不分页
    public function GetDataList($where=array(), $order="comm_like desc")
    {
