...
class Platform extends Model
{
    ...
    //声明添加时间字段
    protected $createTime = 'platform_time';
    //声明修改时间字段
    ...
    protected $rule = [
        'platform_name|平台名称'     => 'require|unique:platform',
    ];
     ...
     **/
    public function GetListByPage($where=array(), $page=1, $limit=10, $order="platform_id desc")
    {   
        return $this->where($where)->page($page,$limit)->order($order)->select();
    }

    //获取数据列表，不分页
    public function GetDataList($where=array(), $order="platform_id desc")
    {
