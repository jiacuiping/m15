 ...
 * 视频分类 表数据模型
 **/
class MusicTrend extends Model
{
    //声明主键
    protected $pk = 'mt_id';
    //自动写入时间戳
    ...
    protected $rule = [
        'mt_music_id|关联音乐表'     => 'require|number',
        'mt_music_number|音乐ID'       => 'require'
    ];
     ...
     **/
    public function GetListByPage($where=array(), $page=1, $limit=10, $order="mt_usercount desc")
    {   
        return $this->where($where)->page($page,$limit)->order($order)->select();
    }

    //获取数据列表，不分页
    public function GetDataList($where=array(), $order="mt_usercount desc")
    {
