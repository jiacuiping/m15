...
class McnAgent extends Model
{
    ...
    //声明表名
    protected $table = 'm15_mcn_agent';
    protected $rule = [
        'agent_name|经纪人姓名' => 'require',
    ];
     ...
     **/
    public function GetListByPage($where=array(), $page=1, $limit=10, $order="agent_id desc")
    {   
        return $this->where($where)->page($page,$limit)->order($order)->select();
    }

    //获取数据列表，不分页
    public function GetDataList($where=array(), $order="agent_id desc")
    {
