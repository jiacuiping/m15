...
class Topic extends Model
{
    ...
    protected $rule = [
        'topic_apiuid|话题发布人'     => 'require|number',
        'topic_number|话题ID'       => 'require|unique:topic'
    ];
     ...
     **/
    public function GetListByPage($where=array(), $page=1, $limit=10, $order="topic_id desc")
    {   
        return $this->where($where)->page($page,$limit)->order($order)->select();
    }

    //获取数据列表，不分页
    public function GetDataList($where=array(), $order="topic_id desc")
    {
