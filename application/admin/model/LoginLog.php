...
class LoginLog extends Model
{
    ...
    protected $rule = [
        'log_user|关联用户'     => 'require|number',
        'log_type|登陆方式'     => 'require'
    ];
     ...
     **/
    public function GetListByPage($where=array(), $page=1, $limit=10, $order="log_id desc")
    {   
        return $this->where($where)->page($page,$limit)->order($order)->select();
    }

    //获取数据列表，不分页
    public function GetDataList($where=array(), $order="log_id desc")
    {
