...
use app\admin\model\Imenu;
    ...
    function GetMenu()
    {
        ...
        $where['imenu_status'] = 1;
        $where['imenu_parent'] = 0;
        ...
        foreach ($menus as $key => $value) {
            ...
            $item = $menuModel->GetListByPage(array('imenu_parent'=>$value['imenu_id'],'imenu_status'=>1));

            if(!empty($item)){
                
                $html .= "  <li data-name='home' class='layui-nav-item'>".
                                "<a href='javascript:;' lay-tips='".$value['imenu_name']."' lay-direction='2'>".
                                    $value['imenu_icon'].
                                    "<cite>".$value['imenu_name']."</cite>".
                                "</a>".
                                "<dl class='layui-nav-child'>";

                foreach ($item as $k => $v) {   //".$v['imenu_url']."
                    $html .= "<dd data-name='console'>".
                                    "<a lay-href='".url($v['imenu_url'])."'>".$v['imenu_name']."</a>".
                                "</dd>";
                }
